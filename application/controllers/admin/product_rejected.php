...
class Product_rejected extends CI_Controller {

    function __construct() {
        parent::__construct();
        if(!$this->common->logged_in()){
            header('location:' . site_url());
        }else if(!$this->common->getPermission($this->session->userdata('primary_email'))->rejected_product){
    ...
    function search() {
        ...
        $data['products'] = $this->common->getAllSellerSearchProductsRejected($sub_category_id);
        $data['competitive'] = $this->common->getCompetitivePrice();
        $data['subcategory'] = $this->common->getSubcategory();
        $this->load->view('admin/header');
        $this->load->view('admin/product/rejected',$data);
