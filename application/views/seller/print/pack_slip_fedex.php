                        ...
                        <tr>
                            ...
                            <td style="padding-left: 20px;">
                                ...
                                <p><?= isset($order->address) ? $order->address . ", " . $order->landmark : '-' ?><br/>
                                    ...
                                    <?= isset($order->state) ? $this->common->getStateName($order->state) : '-' ?> - <?= isset($order->pincode) ? $order->pincode : '-' ?></p>
                            </td>
                        </tr>
                    </table>
                </td>
                    ...
                    <table style="width: 100%;border: 1px solid;">
                        <tr>
                            <td style="width:45%;padding-left: 20px;">                                
                                <h1>FedEx</h1>
                                <p><strong>Shipping Date : <?= isset($order->shipped_date) ? date('d M Y', strtotime($order->shipped_date)) : '-' ?> </strong></p>
                            ...
                            <td>
                                ...
                                if (isset($order->pay_method)) {
                                    if ($order->pay_method == 'cod') {
                                        ?>
                                    &nbsp;&nbsp;COD&nbsp;&nbsp; <?= isset($order->destination_area) ? $order->destination_area : '' ?></h3>
                                    <?php }
                                } ?>
                                <h3>TRK# <?= isset($order->awb_no) ? chunk_split($order->awb_no,'4',' ') : '' ?>&nbsp;&nbsp;&nbsp; Form Id :<?= isset($order->form_id) ? $order->form_id : '' ?></h3>
                                <h3 style="margin-bottom: 0;margin-right: 10px;font-size: 30px;margin-top: 15px;"> <?= isset($order->routing_code) ? $order->routing_code : '-' ?></h3> <h3><?= isset($order->pincode) ? $order->pincode : '-' ?> &nbsp;&nbsp;<?= isset($order->airport_id) ? $order->airport_id : '-' ?> &nbsp;<?=isset($order->pincode)?$this->common->getFedExStateId($order->pincode):''?>-IN</h3><br/>
                                                                
                            </td>
                        </tr>
                        <tr>
                            <td colspan="2" style="text-align:center">
                                <!--<img src="http://barcodes4.me/barcode/c128c/<?= isset($order->barcode_string) ? $order->barcode_string : '' ?>.png?width=241&height=75"/>-->
                                <img src="http://www.barcodesinc.com/generator/image.php?code=<?= isset($order->barcode_string) ? $order->barcode_string : '' ?>&style=68&type=C128C&width=318&height=96&xres=1&font=3" border="0"/>
                                <p><strong>Fedex Meter: 118692262</strong>&nbsp;&nbsp;&nbsp;  Bill T/C: Sender &nbsp;&nbsp;&nbsp; Bill D/T: Recipient</p>
                            </td>
                        </tr>
                    </table>
                </td>
                            ...
                            <table style="width: 100%;border: 1px solid;">
                                <tr>
                                    <td style="width:45%;text-align: center">                                        
                                        <h2 style="margin-top: 50px;">PLEASE COLLECT INR .<?= isset($cod_amount) ? $cod_amount : '0' ?></h2>
                                    </td>
                                    <td>  
                                        <h3>TRK# :<?= isset($order->awb_no_cod) ? chunk_split($order->awb_no_cod,'4',' ') : '' ?>&nbsp;&nbsp;&nbsp; Form Id :<?= isset($order->form_id_cod) ? $order->form_id_cod : '' ?></h3><br/>
                                        <h3><?= isset($order->service_type_cod) ? $order->service_type_cod : '' ?></h3>
                                        <h3>FedEx COD RETURN</h3><br/>                                        
                                    </td>
                                </tr>
                                <tr>
                                    <td colspan="2" style="text-align:center">
                                        <!--<img src="http://barcodes4.me/barcode/c128c/<?= isset($order->barcode_string_cod) ? $order->barcode_string_cod : '' ?>.png?width=241&height=75"/>-->
                                        <img src="http://www.barcodesinc.com/generator/image.php?code=<?= isset($order->barcode_string_cod) ? $order->barcode_string_cod : '' ?>&style=68&type=C128C&width=318&height=96&xres=1&font=3" border="0"/>
                                    </td>
                                </tr>
                            </table>
                        </td>
