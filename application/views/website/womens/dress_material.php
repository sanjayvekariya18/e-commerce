<div class="columns-container">
    <div class="container" id="columns">
        ...
        <div class="breadcrumb clearfix">
            ...
            <span class="navigation_page">Dress Material</span>
        </div>
        <!-- ./breadcrumb -->
        <!-- row -->
        <div class="row">
                ...
                <div class="block left-module">
                    ...
                    <div class="block_content">
                        ...
                        <div class="layered layered-filter-price">                            
                            ...
                            <?php } ?>  
                            <!-- ./filter fabric -->
                            <!-- ./filter bottom fabric -->
                            <?php if (isset($bottom_fabric) && is_array($bottom_fabric)) { ?> 
                                <div class="layered_subtitle">Bottom Fabric</div>
                                <div class="layered-content filter-size">
                                    <ul class="check-box-list">
                                        <?php foreach ($bottom_fabric as $val) { ?>
                                            <li style="width:100%">
                                                <input type="checkbox" id="bottomfabric<?= $val['variation_id'] ?>" name="variation[]" value="<?= $val['variation_id'] ?>" class="myfilter"/>
                                                <label for="bottomfabric<?= $val['variation_id'] ?>">
                                                    <span class="button"></span><?= $val['variation_name'] ?>
                                                </label>   
                                            </li>
                                        <?php } ?>  
                            ...
                            <!-- ./filter bottom fabric -->
                            <!-- ./filter inner fabric -->
                            <?php if (isset($inner_fabric) && is_array($inner_fabric)) { ?> 
                                <div class="layered_subtitle">Inner Fabric</div>
                                <div class="layered-content filter-size">
                                    <ul class="check-box-list">
                                        <?php foreach ($inner_fabric as $val) { ?>
                                            <li style="width:100%">
                                                <input type="checkbox" id="innerfabric<?= $val['variation_id'] ?>" name="variation[]" value="<?= $val['variation_id'] ?>" class="myfilter"/>
                                                <label for="innerfabric<?= $val['variation_id'] ?>">
                                                    <span class="button"></span><?= $val['variation_name'] ?>
                                                </label>   
                                            </li>
                                        <?php } ?>  
                            ...
                            <?php } ?>  
                            <!-- ./filter inner fabric -->
                            <!-- ./filter dupatta fabric -->
                            <?php if (isset($dupatta_fabric) && is_array($dupatta_fabric)) { ?> 
                                <div class="layered_subtitle">Dupatta Fabric</div>
                                <div class="layered-content filter-size">
                                    <ul class="check-box-list">
                                        <?php foreach ($dupatta_fabric as $val) { ?>
                                            <li style="width:100%">
                                                <input type="checkbox" id="dupattafabric<?= $val['variation_id'] ?>" name="variation[]" value="<?= $val['variation_id'] ?>" class="myfilter"/>
                                                <label for="dupattafabric<?= $val['variation_id'] ?>">
                                                    <span class="button"></span><?= $val['variation_name'] ?>
                                                </label>   
                                            </li>
                                        <?php } ?>  
                            ...
                            <?php } ?>  
                            <!-- ./filter pattern -->  
                            <!-- ./filter Celebrity fabric -->
                            <?php if (isset($celebrity) && is_array($celebrity)) { ?> 
                                <div class="layered_subtitle">Celebrity</div>
                                <div class="layered-content filter-size">
                                    <ul class="check-box-list">
                                        <?php foreach ($celebrity as $val) { ?>
                                            <li style="width:100%">
                                                <input type="checkbox" id="celebrity<?= $val['variation_id'] ?>" name="variation[]" value="<?= $val['variation_id'] ?>" class="myfilter"/>
                                                <label for="celebrity<?= $val['variation_id'] ?>">
                                                    <span class="button"></span><?= $val['variation_name'] ?>
                                                </label>   
                                            </li>
                                        <?php } ?>  
            ...
            <div class="center_column col-xs-12 col-sm-9" id="center_column">
                ...
                <div id="view-product-list" class="view-product-list">
                    <h2 class="page-heading">
                        <span class="page-heading-title">Womens Dress Materials</span>
                    </h2>
    ...
    $(document).ready(function() {       
        ...
        $('.myfilter').click(function() {
            ...
            if ($exFlag)
            {
                ...
                $exFlag = 0;
                $.ajax({
                    url: "<?= site_url() ?>womens/dressMaterialFilter",
                    type: "post",
            ...
            if ($(window).scrollTop() + $(window).height() > $(document).height() - 300) {
                if ($exFlag)
                {
                    ...
                    if ($productEnd) {
                        ...
                        if ($isFilter == 0) {
                            $.ajax({
                                url: "<?= site_url() ?>womens/pingDressMaterial",
                                type: "post",
                                data: {'start': $start},
                                success: function(data, textStatus, jqXHR) {
                                    $exFlag = 1;
                            ...
                            $.ajax({
                                url: "<?= site_url() ?>womens/pingFilterDressMaterial",
                                type: "post",
