            ...
            <div class="center_column col-xs-12 col-sm-9" id="center_column">
                ...
                <div id="view-product-list" class="view-product-list">
                    <h2 class="page-heading">
                        <span class="page-heading-title">Search Products</span>
                    </h2>
    ...
    $(document).ready(function () {
        ...
        $('.myfilter').click(function () {
            ...
            if ($exFlag)
            {
                ...
                $exFlag = 0;
                $.ajax({
                    url: "<?= site_url() ?>search/searchFilter",
                    type: "post",
                    data: {'sub_category_id': $sub_category_id, 'keyword': $keyword, 'min_price': $minprice, 'max_price': $maxprice, 'variation_id': $variation_id},
            ...
            if ($(window).scrollTop() + $(window).height() > $(document).height() - 300) {
                if ($exFlag)
                {
                    ...
                    if ($productEnd) {
                        ...
                        if ($isFilter == 0) {
                            $.ajax({
                                url: "<?= site_url() ?>search/pingSearch",
                                type: "post",
                                data: {'sub_category_id': $sub_category_id, 'keyword': $keyword, 'start': $start},
                                success: function (data, textStatus, jqXHR) {
                                    $exFlag = 1;
                            ...
                            $.ajax({
                                url: "<?= site_url() ?>search/pingSearchFilter",
                                type: "post",
                                data: {'sub_category_id': $sub_category_id, 'keyword': $keyword, 'start': $start, 'min_price': $minprice, 'max_price': $maxprice, 'variation_id': $variation_id},
                                success: function (data, textStatus, jqXHR) {
                                    $exFlag = 1;
                                    $('.productlist').append(data);
                                    $('#productload').css('display', 'none');
